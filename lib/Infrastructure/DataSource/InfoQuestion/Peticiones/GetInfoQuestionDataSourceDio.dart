...
class GetInfoQuestionDataSourceDio {
  final Dio dio;

  GetInfoQuestionDataSourceDio({required this.dio});

  Future<ResponseEntity<InfoQuestionModel>> getIntoQuestions({
    required int page,
    required int size,
  }) async {
    try {
      final response = await dio.post(
        "$URLBASE/api/info-question/get-all",
      ...
      if (response.statusCode == 200) {
        List<InfoQuestionModel> questions = InfoQuestionModel.toListModelDio(
          result['data'],
        );
